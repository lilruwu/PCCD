...
void coger_tenedor(int i) {
    ...
    estado = msgrcv(cola[i], &filosofo, sizeof(struct filosofo) - sizeof(long),0,0); //con el ultimo 0 es modo bloqueante si en vez del 0 pones reciebe no wait no se queda suspendido si no hay filosofo
    if (estado == -1) {
        perror("msgrcv");
        exit(EXIT_FAILURE);
    }
